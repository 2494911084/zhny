      ...
      <ul class="products-list product-list-in-box"  style="max-height: 300px;">
          ...
          <li class="item">
              ...
              <div class="product-info">
                <h4><a href="{{ url('admin/nhph?project='.$item['id']) }}" class="{{ $project['id']==$item['id']?'product-title':'text-dark' }}">
                  {{$item['title']}}
                </a></h4>
                <p></p>
                @foreach ($item->equipments()->where('is_ph', 1)->get() as $kk => $equipment)
                <a href="{{ url('admin/nhph?project='.$item['id']) }}" class="text-dark installed">&nbsp;&nbsp;&nbsp;{{ $equipment->name }}</a>
                @endforeach
