      ...
      <ul class="products-list product-list-in-box"  style="max-height: 300px;">
          ...
          <li class="item">
              ...
              <div class="product-info">
                <h4><a href="{{ url('admin/nhdb?project='.$item['id']) }}" class="{{ $project['id']==$item['id']?'product-title':'text-dark' }}">
                  {{$item['title']}}
                </a></h4>
                <p></p>
                @foreach ($item->equipments()->where('is_db',1)->get() as $kk => $equipment)
                <a class="text-dark" href="{{ url('admin/nhdb?project='.$item['id']) }}">&nbsp;&nbsp;&nbsp;{{ $equipment->name }}</a>
                @endforeach
